...
func CreatesolnetworkinstanceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/sol/nslcm/v1/ns_instances", cfg.BaseURL)
		...
		var result models.CreateSolNetworkInstanceOutput
...
func CreateCreatesolnetworkinstanceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_sol_nslcm_v1_ns_instances",
		...
		mcp.WithString("nsDescription", mcp.Description("Input parameter: Network instance description.")),
		mcp.WithString("nsName", mcp.Required(), mcp.Description("Input parameter: Network instance name.")),
		mcp.WithString("nsdInfoId", mcp.Required(), mcp.Description("Input parameter: ID for network service descriptor.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    CreatesolnetworkinstanceHandler(cfg),
	}
}
